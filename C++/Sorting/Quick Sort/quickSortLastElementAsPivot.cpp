...
int partitionForPivot(int arr[],int l,int h)
{
    ...
    swap(arr[i],arr[h]); // arr[h] for pivot. In case, we use pivot as parameter it gives an error because we have to swapping elements of loop. pivot var is static .
